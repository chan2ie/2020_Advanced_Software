...
void CFilteringView::OnProblem3()
{
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			for (k = -w; k <= w; k++) {
				for (l = -w; l <= w; l++) {
					int idx = r * m_width + c + k * m_width + l;
					if (r + k >= 0 && c + l >= 0 && r + k < m_height && c + l < m_width) {
						Gx[r * m_width + c] += double(m_imageBits[idx]) * double(Sx[w + k][w + l]);
						Gy[r * m_width + c] += double(m_imageBits[idx]) * double(Sy[w + k][w + l]);
					}
				}
			}
		...
		for (c=0; c<m_width; c++) {
			...
			for (k = -w; k <= w; k++) {
				for (l = -w; l <= w; l++) {
					...
					double dat;
					if (r + k >= 0 && c + l >= 0 && r + k < m_height && c + l < m_width) {
						dat = imageBits[idx] * kernel[w + k][w + l];
					}
					else {
						dat = 0;
					}
			...
			if (c == 0) {
				...
				for (k = -w; k <= w; k++) {
					for (l = -w; l <= w; l++) {
						...
						double dat;
						if (r + k >= 0 && c + l >= 0 && r + k < m_height && c + l < m_width) {
							dat = m_imageBits[idx] * kernel[w + k][w + l];
						}
						else {
							dat = 0;
						}
				...
				for (k = -w; k <= w; k++) {
					if (r + k >= 0 && r + k < m_height) {
						if (c - w - 1 >= 0) {
							mean -= m_imageBits[(r + k) * m_width + c - w - 1] * kernel[w + k][0];
						}
						if (c + w < m_width) {
							mean += m_imageBits[(r + k) * m_width + c + w] * kernel[w + k][w0 - 1];
						}
					}
				}
			}
...
void CFilteringView::MedianFiltering(int w0)
{
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			for (k = -w; k <= w; k++) {	
				for (l = -w; l <= w; l++) {
					int idx = (r + k) * m_width + c + l;
					if (r + k >= 0 && c + l >=0 && r + k < m_height && c + l < m_width) {
						medarray[i] = m_imageBits[idx];
					}
					else {
						medarray[i] = 0;
					}
