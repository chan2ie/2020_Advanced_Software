...
cudaError_t Fibonacci_GPU(int *h_Fibo_n, int *h_Fibo_v, int *h_Fibo_v_GPU)
{
    ...
    int* d_fib_num, * d_fib_val;

    CUDA_CALL(cudaMalloc(&d_fib_num, N * sizeof(int)));
    CUDA_CALL(cudaMemcpy(d_fib_num, h_Fibo_n, N * sizeof(int), cudaMemcpyHostToDevice));

    CUDA_CALL(cudaMalloc(&d_fib_val, N * sizeof(int)));

    dim3 dimBlock = (BLOCK_SIZE);
    dim3 dimGrid = (N / BLOCK_SIZE);

    CHECK_TIME_START_GPU()
    Fibonacci_Kernel << < dimGrid, dimBlock >> > (d_fib_num, d_fib_val);
    CHECK_TIME_END_GPU(device_time)

    CUDA_CALL(cudaGetLastError())
    CUDA_CALL(cudaDeviceSynchronize())
    CUDA_CALL(cudaMemcpy(h_Fibo_v_GPU, d_fib_val, N * sizeof(int), cudaMemcpyDeviceToHost));
