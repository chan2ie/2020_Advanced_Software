...
cudaError_t Sum_n_elements_GPU(IN int *p_ArrayElements, OUT int *p_SumOfElements_GPU, int Nf) {
    ...
    //array element값을 사용하기 위해 d_ArrayElement를 할당하고 array_elements의 값을 복사하였다.
    CUDA_CALL(cudaMalloc(&d_ArrayElements, N * sizeof(int)));
    CUDA_CALL(cudaMemcpy(d_ArrayElements, p_ArrayElements, N * sizeof(int), cudaMemcpyHostToDevice));

    //계산 결과가 저장될 array를 할당하였다.
    CUDA_CALL(cudaMalloc(&d_SumOfElements_GPU, N * sizeof(int)));

    //BLOCK_SIZE 크기의 1차원 블럭을 사용하였다.
    ...
    CHECK_TIME_END_GPU(device_time)
        ...
        CUDA_CALL(cudaMemcpy(p_SumOfElements_GPU, d_SumOfElements_GPU, N * sizeof(int), cudaMemcpyDeviceToHost));
