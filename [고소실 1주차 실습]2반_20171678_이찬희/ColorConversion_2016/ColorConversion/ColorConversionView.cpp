...
void CColorConversionView::OnProblem1Sub1()
{
	...
	for (int idx = 0; idx < height * width; idx++) {
		mg_lpImage[idx] = m_dibFile.m_lpImage[idx] / 2;
	}
...
void CColorConversionView::OnProblem1Sub2()
{
	...
	for (int idx = 0; idx < height * width; idx++) {
		mg_lpImage[idx] = (m_dibFile.m_lpImage[idx] * 2 > 255) ? 255 : m_dibFile.m_lpImage[idx] * 2;
	}
...
void CColorConversionView::OnProblem2()
{
	...
	for (int idx = 0; idx < height * width; idx++) {
		rd = m_dibFile.m_lpImage[3 * idx];
		gr = m_dibFile.m_lpImage[3 * idx + 1];
		bl = m_dibFile.m_lpImage[3 * idx + 2];
		
		mg_lpImage[idx] = int(0.299 * rd + 0.587 * gr + 0.114 * bl);
	}
...
void CColorConversionView::OnProblem3()
{
	...
	for (int idx = 0; idx < height * width; idx++) {
		r = m_dibFile.m_lpImage[idx * 2];
		c = m_dibFile.m_lpImage[idx * 2 + 1];
	
		rd = (r << 1) >> 3;
		gr = ((r << 6) >> 3) + (c >> 5);
		bl = (c << 3) >> 3;

		int color = int(0.299 * rd + 0.587 * gr + 0.114 * bl);

		mg_lpImage[idx] = color * 255 / 31;
	}
