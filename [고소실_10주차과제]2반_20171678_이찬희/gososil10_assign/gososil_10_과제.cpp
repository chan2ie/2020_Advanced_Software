...
void main(void)
{
	...
	CHECK_TIME_END(compute_time);
	printf("#1 naive calculation = %f ms\n", compute_time);
	CHECK_TIME_START;
	for (int i = 0; i < N_MAX; i++) {
		int r = rand();
	...
	CHECK_TIME_END(compute_time);
	printf("#1 advanced calculation = %f ms\n", compute_time);
	...
	CHECK_TIME_END(compute_time);
	printf("#2 naive calculation = %f ms\n", compute_time);
	CHECK_TIME_START;
	sum = (N_MAX + 1) * N_MAX / 2;
	CHECK_TIME_END(compute_time);
	printf("#2 advanced calculation = %f ms\n", compute_time);
}
...
{	
	...
	if (hw2_b > 0 && abs(hw2_a * hw2_c) / pow(hw2_b, 2) < 0.0001) {(2 * hw2_a);
